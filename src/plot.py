...
#         # Maximum depth
#         self.max_depth = 1 + max(
#             left.max_depth if left else 0,
#             right.max_depth if right else 0
#         )
...
def _plot_dtree_node(node, min_x: float) -> tuple[float, tuple[float, float]]:
    (lb_max_x, (lb_root_x, lb_root_y)) = _plot_dtree_node(
        node.left, min_x) if not node.is_leaf() else (min_x, (None, None))
    (root_x, root_y) = (lb_max_x + PADDING_X, node.max_depth * PADDING_Y)

    plt.text(root_x, root_y, node.get_label_txt(),
             bbox=LABEL_BBOX_STYLE, ha='center', va='center')

    (rb_max_x, (rb_root_x, rb_root_y)) = _plot_dtree_node(
        node.right, root_x) if not node.is_leaf() else (root_x, (None, None))
...
def plot_dtree(tree, filename: str):
    plt.figure()

    font_size = 150 / tree.size  # tree.count_nodes()
    font_size_dpi = max(600 / font_size, 180)

    plt.rcParams.update({'font.size': font_size})

    (max_x, _) = _plot_dtree_node(tree, 0)
    max_y = (tree.max_depth - 1) * PADDING_Y
...
def plot_tree_node(node, x: int):

    y = node.max_depth * HEIGHT_OFFSET
...
def plot_decision_tree(tree, max_depth: int):
